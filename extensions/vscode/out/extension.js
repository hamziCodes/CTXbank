...
function activate(context) {
    ...
    // 3. Register Commands
    context.subscriptions.push(vscode.commands.registerCommand('ctxbank.refresh', () => {
...
function openVisualDashboard(context, workspaceRoot) {
    const panel = vscode.window.createWebviewPanel('ctxbankDashboard', 'CTXbank: Architecture Tree', vscode.ViewColumn.One, { enableScripts: true });
    panel.webview.html = getWebviewContent(workspaceRoot);
}
function getWebviewContent(workspaceRoot) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
async function updateStatusBar(workspaceRoot, item) {
    const activePath = path.join(workspaceRoot, 'memory-bank', 'activeContext.md');
    try {
        const data = await vscode.workspace.fs.readFile(vscode.Uri.file(activePath));
        const lines = Buffer.from(data).toString('utf8').split('\n').length;
...
function runCtxCommand(args, focus, notes) {
    const terminal = vscode.window.createTerminal('CTXbank');
    terminal.show();
